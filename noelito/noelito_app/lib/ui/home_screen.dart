...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Future<void> _init() async {
    ...
    final running = await _wake.isRunning();
    if (mounted) setState(() => _wakeActive = running);

    // Si la app se abrió porque se detectó "Oye Noelito", arranca a escuchar de una vez.
    final triggered = await _wake.consumeWakeFlag();
    if (triggered && mounted) {
      ref.read(assistantProvider.notifier).listenAndRespond();
    }
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: _wakeActive
                ? 'Desactivar "Oye Noelito"'
                : 'Activar "Oye Noelito"',
            icon: Icon(
              _wakeActive ? Icons.record_voice_over : Icons.voice_over_off,
              color: _wakeActive ? scheme.primary : null,
            ),
